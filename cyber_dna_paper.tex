...
\author[1]{\fnm{Sindhoora} \sur{R}}\email{sindhoora807@gmail.com}
\author[1]{\fnm{Nivedita V} \sur{Shetty}}\email{niviishetty.18@gmail.com}
\author[1]{\fnm{Sri Krishna} \sur{Tilakraj}}\email{thilakrajkc@gmail.com}
\author[1]{\fnm{Shama S} \sur{Bangera}}\email{shamabangera21@gmail.com}
\author[1]{\fnm{Sushanth S} \sur{Shetty}}\email{sushanthsshetty37@gmail.com}
\author[2]{\fnm{Shrikanth} \sur{Prabhu}}
\author[1]{\fnm{Ashmi} \sur{Kotian}}\email{ashmi.kotian@nitte.edu.in}

\affil[1]{\orgdiv{Department of Cyber Security}, \orgname{NMAM Institute of Technology (NMAMIT), Nitte (Deemed to be University)}, \orgaddress{\city{Nitte, Udupi}, \postcode{574110}, \state{Karnataka}, \country{India}}}
\affil[2]{\orgdiv{Department of Computer Science and Engineering}, \orgname{NMAM Institute of Technology (NMAMIT), Nitte (Deemed to be University)}, \orgaddress{\city{Nitte, Udupi}, \postcode{574110}, \state{Karnataka}, \country{India}}}
...
\subsection{Heuristic Baseline Comparison}\label{subsec5_1}
To validate the necessity of the machine learning classifier, we compare Cyber DNA against a simple threshold-based heuristic baseline. The heuristic flags any user-week in which either the logon frequency or the number of USB transfers exceeds three standard deviations ($>3\sigma$) of that user's historical rolling average. This simple heuristic yields a recall of 58.54\% (48 TP), but a precision of only 1.15\% (4,128 FP), resulting in a poor F1-score of 2.25\% due to the high volume of false alarms. This highlights the value of the non-linear feature interaction modeling in the XGBoost architecture.

\subsection{Comparison with State-of-the-Art Methods}\label{subsec5_2}
To address the performance gap reported between Cyber DNA and other published systems, we compare our chronological out-of-sample metrics against literature values in Table~\ref{tab:sota}.

\begin{table}[h]
\caption{Comparison of Cyber DNA with state-of-the-art literature on the CMU CERT r4.2 dataset.}\label{tab:sota}
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lccc}
\toprule
Method & Model Type & Split Type & F1-Score \\
\midrule
Tuor et al. \cite{tuor2017deep} & Unsupervised LSTM & Random Split & 82.10\% \\
Yuan et al. \cite{yuan2021deep} & Supervised Bi-LSTM & Random Split & 85.30\% \\
Heuristic Baseline & $3\sigma$ Outlier Rules & Chronological & 2.25\% \\
\textbf{Cyber DNA (Ours)} & XGBoost Classifier & Chronological & \textbf{48.41\%} \\
\bottomrule
\end{tabular*}
\end{table}

While deep neural architectures claim higher F1-scores, they are evaluated under randomized cross-validation schemes that leak future behavioral baselines into the training process. When evaluated under a strict, chronologically clean split, performance drops significantly due to true temporal concept drift. Cyber DNA represents the first evaluated baseline under this strict, leakage-free environment.
...
\subsection{The Role of Consistency Metrics as Guardrail Features}\label{subsec6_1}
A key concern raised in the feature ablation analysis is that the high-level longitudinal consistency features—Identity Persistence (IDP), Behavioral Continuity (BC), and Social Role Consistency (SRC)—receive near-zero gain importance when evaluated in isolation. This behavior is characteristic of "guardrail" features. Rather than functioning as direct predictors of malicious activity (which are heavily dominated by active off-hours and USB spikes), these metrics baseline a user's normal degree of behavioral variability. By providing the XGBoost model with these consistency scores, the classifier is able to contextualize behavioral spikes. If a user exhibits a high-drift score but has historically low behavioral continuity (reflecting a naturally variable role), the guardrail prevents a false positive. This contributes significantly to the final precision improvement under the expanded feature set.

\subsection{Operational Triage in the Security Operations Center}\label{subsec6_2}
...
\subsection{Transferability and Real-World Deployment Path}\label{subsec7_1}
To transition the Cyber DNA framework from the CMU CERT simulation platform to a real-world enterprise deployment, several operational adjustments must be made:
\begin{itemize}
    \item \textbf{Data Log Ingestion}: Raw logon and logoff events can be pulled directly from Active Directory Windows Security Event Logs (Event IDs 4624 and 4625), while email metadata can be collected from Exchange or Google Workspace message logs. Removable media activity is tracked using endpoint logs (Event ID 2003/2004 for USB connections).
    \item \textbf{Rolling Window Computation}: To resolve the latency limitations of fixed weekly boundaries, we propose a rolling 7-day window updated daily. Under this design, behavioral signature vectors ($\mathbf{DBS}$) are updated at the end of each business day based on the preceding 7-day window.
    \item \textbf{Dynamic Profiling & Baseline Calibration}: Rather than static thresholds, normalization bounds and the base signature reference ($T_{\text{base}}$) must undergo periodic online updating (e.g., recalculating bounds every quarter) to account for natural long-term career progression and institutional role shifts.
\end{itemize}
